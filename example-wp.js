/*============================================================================*\
  Example of webserver using PHP and websockets served on the same port
...
sphp.maxSpareWorkers =20;
sphp.serverName = 'localhost';
sphp.serverName = 'example.com';
...
app.use(function(request, response, next){ 
  if(request._parsedUrl.pathname.slice(-1) == '/') 
    request._parsedUrl.pathname += '/index.php';
  next();
});

// Attach sPHP
app.use(sphp.express(thisPath+'/wp/'));

// Attach sPHP execution to the websocket connect event
ws.on('connection',sphp.websocket(sessionOptions));

app.use(express.static(thisPath+'/wp/'));
